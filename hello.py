...
def result():
	if request.method == 'POST':
		...
		for i in range(len(dataset)) : 
			if dataset.loc[i, "Suggested Job Role"]=='Network Security Administrator' or dataset.loc[i, "Suggested Job Role"]=='Network Engineer' or dataset.loc[i, "Suggested Job Role"]=='Network Security Engineer':
				dataset.loc[i, "Suggested Job Role"]='Network Admin'
			if dataset.loc[i, "Suggested Job Role"]=='Business Systems Analyst' or dataset.loc[i, "Suggested Job Role"]=='Business Intelligence Analyst' or dataset.loc[i, "Suggested Job Role"]=='CRM Business Analyst':
				dataset.loc[i, "Suggested Job Role"]='Business Analyst'
			if dataset.loc[i, "Suggested Job Role"]=='Database Manager' or dataset.loc[i, "Suggested Job Role"]=='Database Administrator':
				dataset.loc[i, "Suggested Job Role"]='Database Developer'
			if dataset.loc[i, "Suggested Job Role"]=='Information Security Analyst' or dataset.loc[i, "Suggested Job Role"]=='Information Technology Auditor' or dataset.loc[i, "Suggested Job Role"]=='Information Technology Manager':
				dataset.loc[i, "Suggested Job Role"]='IT Admin'
			if dataset.loc[i, "Suggested Job Role"]=='Software Systems Engineer' or dataset.loc[i, "Suggested Job Role"]=='Software Developer' or dataset.loc[i, "Suggested Job Role"]=='Software Quality Assurance (QA) / Testing':
				dataset.loc[i, "Suggested Job Role"]='Software Engineer'
			if dataset.loc[i, "Suggested Job Role"]=='CRM Technical Developer' or dataset.loc[i, "Suggested Job Role"]=='Technical Support' or dataset.loc[i, "Suggested Job Role"]=='Technical Services/Help Desk/Tech Support':
			...
			if dataset.loc[i, "Suggested Job Role"]=='Portal Administrator' or dataset.loc[i, "Suggested Job Role"]=='Systems Security Administrator':
				dataset.loc[i, "Suggested Job Role"]='Administrator'
			if dataset.loc[i, "Suggested Job Role"]=='Mobile Applications Developer':
		...
		input1 = np.array([[int(pos),int(pa),int(pse),int(pcn),int(pe),int(pcoa),int(pm),int(comm),int(hrs),int(logical),int(hack),int(coding),int(speak),screen,learn,extra,wc1,wc2,rw,mem,is1,ica,job,comp,mt,team,ie]])
		data1=data
	...
	return render_template('result.html',var1=res[0][0],var2=res[1][0],var3=res[2][0])
